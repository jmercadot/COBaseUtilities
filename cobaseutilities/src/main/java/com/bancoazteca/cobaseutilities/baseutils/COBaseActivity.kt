...
open class COBaseActivity : AppCompatActivity() {


    private lateinit var progressDialog: Dialog

    fun mostrarFragmentFade(
        fragmentIn: COBaseFragment,
        containerViewId: Int,
        bundle: Bundle?,
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent): Boolean {
        ...
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            for (i in supportFragmentManager.fragments.indices) {
                if (supportFragmentManager.fragments[i] is COBaseFragment) {
                    val fragment =
                        supportFragmentManager.fragments[i] as COBaseFragment
